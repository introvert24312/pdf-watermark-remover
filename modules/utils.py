...
def remove_watermark(images, mask, parameters):
    ...
    print(f"\n{'='*50}", flush=True)
    print(f"Starting watermark removal for {total_images} images...", flush=True)
    print(f"{'='*50}\n", flush=True)
    ...
    for i, img in enumerate(images):
        ...
        print(f"Processing image {i+1}/{total_images} ({progress*100:.1f}%)", flush=True)
        ...
        # Show the image with progress bar (may not work in PyQt5 environment)
        try:
            cv2.imshow('Removing watermark...', image_with_progress)
            cv2.waitKey(1)
        except:
            pass  # Silently fail if cv2.imshow doesn't work
    ...
    print(f"\n{'='*50}", flush=True)
    print(f"Watermark removal complete!", flush=True)
    print(f"{'='*50}\n", flush=True)
