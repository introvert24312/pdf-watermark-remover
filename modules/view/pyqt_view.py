...
class SidebarBuilder:
    ...
    def create_trackbars(self, trackbars):
        for name, value in trackbars.items():
            ...
            label = QLabel(name)
            label.setFixedHeight(20)  # Fixed height to prevent jumping
            self.slider_labels[name] = label  # Store label reference
            self.sidebar.layout().addWidget(label)

            slider = QSlider(Qt.Horizontal)
            slider.setMinimum(range_min)
            slider.setMaximum(range_max)
            slider.setValue(int(value['value']))
            slider.setFixedHeight(25)  # Fixed height to prevent jumping

            # Disable tracking to only update on release (prevents lag during drag)
            # Or keep tracking enabled but the update is optimized
            slider.setTracking(False)  # Only emit signal when released

            # Store reference to the slider
            self.sliders[name] = slider

            # Connect callback - now only fires when slider is released
            ...
            self.sidebar.layout().addWidget(slider)
    ...
    def _bind_mouse_events(self, mouse_callback):
        ...
        def mouse_handler(event, event_type_str):
            try:
                ...
                # Calculate the actual displayed size (QLabel centers and scales the pixmap)
                # The pixmap maintains aspect ratio
                ...
                # Calculate offset (pixmap is centered in the label)
        ...
        if hasattr(self, 'sliders'):
            for i in range(len(values['names'])):
                ...
                if name in self.sliders:
                    # Block signals to prevent callback loops
